...
const EmployeeDetail: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      let url = `${import.meta.env.VITE_API_URL || 'http://127.0.0.1:5000'}/api/employee?`;
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:5000'}/api/employee/${data.selected_emp.emp_code}`, { method: 'DELETE' });
      if (res.ok) {
